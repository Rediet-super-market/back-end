...
Class Order extends Controller
{
	...
	public function save_order($POST,$ROWS,$user_url,$sessionid)
	{
		...
		if(is_array($ROWS) && count($this->errors) == 0){
			...
			$query = "insert into order (description,user_url,delivery_address,total,country,state,zip,tax,shipping,date,sessionid) 
			values 
			(:description,:user_url,:delivery_address,:total,:country,:state,:zip,:tax,:shipping,:date,:sessionid)";
			...
			foreach ($ROWS as $row) {
				...
				$data['quantity'] = $row->cart_qty;
				$data['description'] = $row->description;
				$data['amount'] = $row->price;
				$data['total'] = $row->cart_qty * $row->price;
				$data['productid'] = $row->id;

				$query = "insert into order_details (orderid,quantity,description,amount,total,productid) values (:orderid,:quantity,:description,:amount,:total,:productid)";
				$result = $db->write($query,$data);
			}
		}
	}
	...
	public function get_orders_by_user($user_url){
		...
		$query = "select * from order where user_url = :user_url order by id desc limit 100";
	...
	public function get_orders_count($user_url){
		...
		$query = "select id from order where user_url = :user_url ";
	...
	public function get_all_orders(){
		...
		$query = "select * from order order by id desc limit $limit offset $offset";
